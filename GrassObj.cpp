...
ID3D12Device* GrassObj::device = nullptr;
UINT GrassObj::descriptorHandleIncrementSize = 0;
ID3D12GraphicsCommandList* GrassObj::cmdList = nullptr;
ComPtr<ID3D12RootSignature> GrassObj::rootsignature;
ComPtr<ID3D12PipelineState> GrassObj::pipelinestate;
ComPtr<ID3D12DescriptorHeap> GrassObj::descHeap;
ComPtr<ID3D12Resource> GrassObj::vertBuff;
//ComPtr<ID3D12Resource> Object3d::indexBuff;
ComPtr<ID3D12Resource> GrassObj::texbuff;
CD3DX12_CPU_DESCRIPTOR_HANDLE GrassObj::cpuDescHandleSRV;
CD3DX12_GPU_DESCRIPTOR_HANDLE GrassObj::gpuDescHandleSRV;
XMMATRIX GrassObj::matView{};
XMMATRIX GrassObj::matProjection{};
XMFLOAT3 GrassObj::eye = { 0, 0, -5.0f };
XMFLOAT3 GrassObj::target = { 0, 0, 0 };
XMFLOAT3 GrassObj::up = { 0, 1, 0 };
D3D12_VERTEX_BUFFER_VIEW GrassObj::vbView{};
//D3D12_INDEX_BUFFER_VIEW Object3d::ibView{};
GrassObj::VertexPos GrassObj::vertices[vertexCount];
//unsigned short Object3d::indices[indexCount];

XMMATRIX GrassObj::matBillboard = XMMatrixIdentity();
XMMATRIX GrassObj::matBillboardY = XMMatrixIdentity();

void GrassObj::StaticInitialize(ID3D12Device* device, int window_width, int window_height)
{
	// nullptrチェック
	assert(device);

	GrassObj::device = device;
...
void GrassObj::PreDraw(ID3D12GraphicsCommandList* cmdList)
{
...
void GrassObj::SetEye(XMFLOAT3 eye)
{
	GrassObj::eye = eye;
...
void GrassObj::SetTarget(XMFLOAT3 target)
{
	GrassObj::target = target;

	UpdateViewMatrix();
}

void GrassObj::CameraMoveVector(XMFLOAT3 move)
{
...
void GrassObj::InitializeDescriptorHeap()
{
...
void GrassObj::InitializeCamera(int window_width, int window_height)
{
	//// ビュー行列の生成
...
void GrassObj::InitializeGraphicsPipeline()
{
	HRESULT result = S_FALSE;
...
void GrassObj::Update(bool isBillboard)
{
	HRESULT result;
